...
var AffCommissionRate = 0.0     // 充值返佣比例，如 0.05 = 5%
var AffCommissionExpireDays = 0 // 返佣有效期天数，0 = 永久

// 邀请返利配置的服务端上界。写入路径必须校验，使用路径再做一次钳制，
// 以免历史库中的越界值把返佣金额算成溢出值。
const (
	// MaxAffCommissionRate 与前端表单一致，返佣比例上限 20%。
	MaxAffCommissionRate = 0.2
	// MaxAffCommissionExpireDays 限制返佣有效期的最大天数（100 年），
	// 避免注册时间 + 天数 * 86400 在 int64 上溢出。
	MaxAffCommissionExpireDays = 36500
	// MaxAffQuotaReward 限制单次邀请奖励额度：aff_quota/aff_history 是 INT 列，
	// 超过 MaxQuota 的值在 MySQL/PostgreSQL 上必然写入失败。
	MaxAffQuotaReward = MaxQuota
)

...
func InitOptionMap() {
	...
	common.OptionMap["AffCommissionExpireDays"] = strconv.Itoa(common.AffCommissionExpireDays)
...
func validateOptionValue(key string, value string) error {
	...
	if key == "QuotaForInviter" || key == "QuotaForInvitee" {
		...
		if err != nil || reward < 0 || reward > common.MaxAffQuotaReward {
			return fmt.Errorf("%s 必须为 0 ~ %d 之间的整数", key, common.MaxAffQuotaReward)
		}
		return nil
	}
	if key == "AffCommissionRate" {
		rate, err := strconv.ParseFloat(strings.TrimSpace(value), 64)
		if err != nil || math.IsNaN(rate) || math.IsInf(rate, 0) || rate < 0 || rate > common.MaxAffCommissionRate {
			return fmt.Errorf("AffCommissionRate 必须为 0 ~ %v 之间的数值", common.MaxAffCommissionRate)
		}
		return nil
	}
	if key == "AffCommissionExpireDays" {
		days, err := strconv.Atoi(strings.TrimSpace(value))
		if err != nil || days < 0 || days > common.MaxAffCommissionExpireDays {
			return fmt.Errorf("AffCommissionExpireDays 必须为 0 ~ %d 之间的整数", common.MaxAffCommissionExpireDays)
		}
		return nil
	}
	return nil
...
func updateOptionMap(key string, value string) (err error) {
	...
	case "AffCommissionExpireDays":
		common.AffCommissionExpireDays, _ = strconv.Atoi(value)

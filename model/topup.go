...
func RechargeEpay(tradeNo string, actualPaymentMethod string, callerIp string) (alreadyDone bool, err error) {
	...
	CreditAffiliateCommission(topUp.UserId, quotaToAdd)
	return false, nil
}
...
func Recharge(referenceId string, customerId string, callerIp string) (err error) {
	...
	CreditAffiliateCommission(topUp.UserId, quota)
...
func ManualCompleteTopUp(tradeNo string, callerIp string) error {
	...
	CreditAffiliateCommission(userId, quotaToAdd)
	return nil
}
...
// CreditAffiliateCommission 根据充值额度计算并发放返佣给邀请人。
func CreditAffiliateCommission(userId int, topupQuota int) {
	if topupQuota <= 0 || !operation_setting.IsPaymentComplianceConfirmed() {
		return
	}
	...
	// 防御历史库中的越界配置：比例上限与写入校验、前端表单保持一致。
	if rate > common.MaxAffCommissionRate {
		common.SysError(fmt.Sprintf("affiliate commission rate %v exceeds %v, clamped", rate, common.MaxAffCommissionRate))
		rate = common.MaxAffCommissionRate
	}
	var topUpUser User
	if err := DB.Select("id, inviter_id, created_at").Where("id = ?", userId).First(&topUpUser).Error; err != nil {
		return
	}
	if topUpUser.InviterId == 0 {
		return
	}
	// 有效期检查
	expireDays := common.AffCommissionExpireDays
	if expireDays > common.MaxAffCommissionExpireDays {
		// 历史库中的越界天数会让 天数 * 86400 溢出，进而把返佣静默关掉
		common.SysError(fmt.Sprintf("affiliate commission expire days %d exceeds %d, clamped", expireDays, common.MaxAffCommissionExpireDays))
		expireDays = common.MaxAffCommissionExpireDays
	}
	if expireDays > 0 {
		expireAt := topUpUser.CreatedAt + int64(expireDays)*86400
		if common.GetTimestamp() > expireAt {
			return
		}
	}
	// 返佣是额度乘法，必须走带饱和的换算，禁止裸 int() 转换。
	commission, clamp := common.QuotaFromFloatChecked(float64(topupQuota) * rate)
	if clamp != nil {
		common.SysError(fmt.Sprintf("affiliate commission for top-up user %d saturated: %s", userId, clamp.Error()))
	}
	if commission <= 0 {
		return
	}
	if err := creditAffQuota(topUpUser.InviterId, commission, nil); err != nil {
		common.SysError(fmt.Sprintf("failed to credit affiliate commission to inviter %d: %s", topUpUser.InviterId, err.Error()))
		return
	}
	RecordLog(topUpUser.InviterId, LogTypeSystem,
		fmt.Sprintf("充值返佣：被邀请用户 %d 充值 %s，获得返佣 %s",
			userId, logger.LogQuota(topupQuota), logger.LogQuota(commission)))
}

...
type User struct {
	...
	AffCode              string                     `json:"aff_code" gorm:"type:varchar(32);column:aff_code;uniqueIndex" validate:"max=32"`
	AffCount             int                        `json:"aff_count" gorm:"type:int;default:0;column:aff_count"`
	AffQuota             int                        `json:"aff_quota" gorm:"type:int;default:0;column:aff_quota"`           // 邀请剩余额度
	AffHistoryQuota      int                        `json:"aff_history_quota" gorm:"type:int;default:0;column:aff_history"` // 邀请历史额度
	InviterId            int                        `json:"inviter_id" gorm:"type:int;column:inviter_id;index"`
	AffiliateClientIP    string                     `json:"-" gorm:"-:all"` // registration client IP for dedup, not persisted
...
var ErrAffCodeNotFound = errors.New("邀请码不存在")
...
func GetUserIdByAffCode(affCode string) (int, error) {
	...
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return 0, ErrAffCodeNotFound
	}
	return user.Id, err
}

// generateUniqueAffCode 生成未被占用的邀请码。查询必须包含软删除行：
// aff_code 的唯一索引同样覆盖它们，撞上就会让用户写入直接失败。
func generateUniqueAffCode(tx *gorm.DB) string {
	for _, length := range []int{6, 6, 6, 10, 16} {
		code := common.GetRandomString(length)
		var count int64
		if err := tx.Unscoped().Model(&User{}).Where("aff_code = ?", code).Count(&count).Error; err == nil && count == 0 {
			return code
		}
	}
	// 连续撞码说明随机源或查询异常，退化为更长的一次性随机码
	return common.GetRandomString(32)
}

// EnsureAffCode 为用户补齐邀请码并返回，已有邀请码时直接返回原值。
// 用于兼容历史数据中没有邀请码的账号，重复调用不会再生成新码。
func (user *User) EnsureAffCode() (string, error) {
	if user.Id == 0 {
		return "", errors.New("id 为空！")
	}
	if user.AffCode != "" {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		var current User
		if err := lockForUpdate(tx).Select("id", "aff_code").First(&current, user.Id).Error; err != nil {
			return err
		}
		if current.AffCode != "" {
		...
		code = generateUniqueAffCode(tx)
		return tx.Model(&User{}).Where("id = ?", user.Id).Update("aff_code", code).Error
	})
	if err != nil {
		return "", err
	}
...
// creditAffQuota 在 aff_quota/aff_history 未越界时累加邀请额度。
// 两列都是 INT 列，必须带条件更新：否则越界累加会在 MySQL/PostgreSQL 上报错，
// 而 SQLite 会成功写入，导致三库行为不一致。
func creditAffQuota(inviterId int, reward int, extraUpdates map[string]any) error {
	...
	result := DB.Model(&User{}).
		Where("id = ? AND aff_quota <= ? AND aff_history <= ?", inviterId, common.MaxAffQuotaReward-reward, common.MaxAffQuotaReward-reward).
		Updates(updates)
	if result.Error != nil {
		return result.Error
	}
	if result.RowsAffected == 0 {
		return fmt.Errorf("邀请额度已达上限 %d", common.MaxAffQuotaReward)
	}
	return nil
}

func inviteUser(inviterId int) error {
	return creditAffQuota(inviterId, common.QuotaForInviter, map[string]any{
		"aff_count": gorm.Expr("aff_count + ?", 1),
	})
}

// awardInviteRewards 在注册完成后发放邀请奖励。
// 同一 (IP, 邀请人) 组合 24 小时内只发放一次，且该去重同时覆盖被邀请人奖励：
// 只拦截邀请人奖励时，同一个人仍能反复用自己的邀请码注册小号刷取被邀请人奖励。
// 代价是共享出口 IP（办公室/NAT）下 24 小时内的后续注册拿不到被邀请人奖励。
func awardInviteRewards(inviteeId int, inviterId int, clientIP string) {
	...
	if clientIP != "" && !claimInviteRewardSlot(inviterId, clientIP) {
		return
	}
	if common.QuotaForInvitee > 0 {
		if err := IncreaseUserQuota(inviteeId, common.QuotaForInvitee, true); err != nil {
			common.SysError(fmt.Sprintf("failed to grant invitee reward to user %d: %s", inviteeId, err.Error()))
		} else {
	...
	if common.QuotaForInviter > 0 {
		if err := inviteUser(inviterId); err != nil {
			common.SysError(fmt.Sprintf("failed to grant inviter reward to user %d: %s", inviterId, err.Error()))
		} else {
...
func (user *User) TransferAffQuotaToQuota(quota int) error {
	...
	// 预扣以缓存余额为准，授信提交后必须把增量补进缓存，
	// 否则转入的额度在缓存过期前不可用。
	syncCreditUserQuotaCache(user.Id, quota, "affiliate quota transfer")
	RecordLog(user.Id, LogTypeSystem, fmt.Sprintf("邀请额度转入余额 %s", logger.LogQuota(quota)))
	return nil
}
...
func (user *User) Insert(inviterId int) error {
	if err := DB.Transaction(func(tx *gorm.DB) error {
		return withNormalizedEmailLock(tx, user.Email, func(tx *gorm.DB) error {
			...
			user.AffCode = generateUniqueAffCode(tx)
...
func (user *User) finishInsert(inviterId int) {
	...
	awardInviteRewards(user.Id, inviterId, user.AffiliateClientIP)
}

...
| `per_call` | `per_call(amount) → float64` | Alias of `fixed` for price lists that spell per-request prices as per-call prices |
...
### Request Prices (`fixed` / `per_call`)

`tier("request", fixed(0.01))` replaces all token charges in the selected leaf
with a $0.01 base price for one successful HTTP/SSE request. `per_call` is an
interchangeable alias, so a per-request price may be written either way. Other
leaves may still use token pricing. Group ratios and request multipliers
continue to apply; existing tool surcharges are calculated separately and added
as before. A stream does not incur a fixed fee per chunk. Realtime requests
reject `fixed()`/`per_call()` before upstream submission or reservation.

The amount must be a finite, non-negative numeric literal whose v1 scaled value
is finite. Explicit `fixed(0)` is valid and stays free. Expressions using a
request price must consist of a conditional pricing tree and the standard
request multiplier factors described below, with each leaf wrapped in `tier()`.
Adding request and token charges in one leaf, adding multiple tiers together, or
multiplying a request price by tokens is rejected. Validation examines the
original AST, including branches that optimization or short-circuiting would
skip. Existing expressions without `fixed()` retain their original grammar and
behavior.

#### Request multipliers

A request price is scaled by factors appended after the leaf:

```
# Literal factor: traced as one request rule per value.
tier("per_second", per_call(0.05)) * (param("seconds") == "8" ? 8 : 1)

# Dynamic factor: reads one bounded request value at run time.
tier("per_second", per_call(0.05)) * param("seconds")

# Resolution tiers and image count.
param("resolution") == "2k" ? tier("2k", per_call(0.03)) : tier("4k", per_call(0.05))
tier("1k", per_call(0.03)) * param("n")
```

Factors must be applied **outside** `tier(...)`; a multiplication inside the
leaf is rejected. Because `*` binds tighter than `?:`, a factor meant to scale a
whole multi-branch tree must wrap that tree in parentheses — otherwise it scales
only the last `else` branch:

```
# Scales every tier by the requested image count.
(param("size") == "1024x1024"
  ? tier("1k", per_call(0.03))
  : (param("size") == "4096x4096" ? tier("4k", per_call(0.05)) : tier("2k", per_call(0.03)))
) * param("n")
```
...
|------|-----------|---------------|
| `1k` | ≤ 1600 | `1024x1024`, `1280x720`, `1344x768`, `1536x1024` |
...
- **Literal**: `<probe condition> ? <number> : 1`, where the condition references
  at least one request probe (`param`, `header`, `hour`, `minute`, `weekday`,
  `month`, `day`). Each literal is traced so the consume log lists the rule that
  applied.
- **Dynamic**: exactly one `param("path")`, `header("key")`, or `u("key")` call
  with a literal argument. The value is converted to a multiplier at run time and
  traced as a `dynamic` rule carrying the value actually applied.

Dynamic factors are bounded. A value that is absent, empty, or `0` is neutral
(`1`) so the documented API default for count and duration fields applies; a
negative, non-finite, or non-numeric value fails the evaluation instead of
charging a guess; and a value above `MaxRequestMultiplier` (3600, matching the
host's task duration bound) is saturated and flagged as `clamped` on the rule.
Hosts still validate the underlying request fields (video `seconds`, image `n`)
before pricing — the engine ceiling is the last line of defense.

`fixed(amount)` returns `amount * 1,000,000` internally, preserving v1's existing
quota conversion and rounding. Request-priced leaves keep that representation on
the task path too, so `tier("per_second", per_call(0.05)) * param("seconds")`
means $0.05 per second whether the model is relayed synchronously or as a task.
...
Task models also accept request-priced leaves, which price from the submitted
request instead of (or in addition to) declared usage facts:

```
# Per-second task price from the request body.
tier("per_second", per_call(0.05)) * param("seconds")

# Per-second task price from a declared usage fact.
tier("per_second", per_call(0.05)) * u("seconds")

# Resolution tier times declared seconds.
u("resolution") == "1080p" ? tier("1080p", per_call(0.6)) * u("seconds") : tier("720p", per_call(0.4)) * u("seconds")
```

The request body is available at submission but not when a task completes, so
the literal `param()` paths an expression reads are frozen on the billing
snapshot (`param_facts`) at submission and replayed at settlement; a
request-derived price therefore cannot change between the two evaluations.
`u()` facts keep their overlay semantics: measured completion values replace the
submission estimates key by key.

...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	if useTiered {
		...
		// The request body is available here but not at completion, so param()
		// probes are read now and frozen onto the snapshot for settlement.
		requestInput, inputErr := helper.ResolveIncomingBillingExprRequestInput(c, info)
		if inputErr != nil {
			return nil, service.TaskErrorWrapperLocal(inputErr, "model_price_error", http.StatusBadRequest)
		}
		requestInput.Usage = facts
		cost, trace, runErr := billingexpr.RunExprWithRequest(exprStr, billingexpr.TokenParams{}, requestInput)
		...
		quota, clamp := common.QuotaRoundChecked(
			billingexpr.USDExpressionOutput(cost, true, trace.BillingUnit) * common.QuotaPerUnit * groupRatioInfo.GroupRatio,
		)
		...
		info.TieredBillingSnapshot = &billingexpr.BillingSnapshot{
			...
			EstimatedQuotaBeforeGroup: billingexpr.USDExpressionOutput(cost, true, trace.BillingUnit) * common.QuotaPerUnit,
			...
			UsageFacts:                facts,
			ParamFacts:                billingexpr.FreezeParamFacts(exprStr, requestInput),
		}
	} else {

...
const requestParamTaskPlugin = `
export const meta = {apiVersion:1,key:"param-price",name:"Param Price",version:"1.0.0",author:{name:"Test"},models:["param-price-model"],fetchMode:"per_task",usageSchema:{seconds:{type:"number",unit:"second"}}};
export function extractUsage(ctx){ return {seconds: Number((ctx.requestBody||{}).seconds || 0)}; }
export function buildSubmitRequest(ctx){ return {url: ctx.baseUrl+"/submit", method:"POST", body:{model: ctx.model, seconds: (ctx.requestBody||{}).seconds}}; }
export function parseSubmitResponse(){return {taskId:"1"};}
export function buildQueryRequest(){return {url:"https://provider.example"};}
export function parseTaskResult(){return {status:"SUCCESS"};}
`

// Task pricing can read the submitted request body: param() values are frozen on
// the snapshot at submission because the body is gone at completion, and
// settlement must reproduce the submission charge exactly.
func TestRelayTaskSubmitPricesFromRequestBodyParams(t *testing.T) {
	saveBillingConfig(t)
	const expression = `tier("per_second", per_call(0.05)) * param("seconds")`
	modeJSON, err := common.Marshal(map[string]string{"param-price-model": billing_setting.BillingModeTieredExpr})
	require.NoError(t, err)
	exprJSON, err := common.Marshal(map[string]string{"param-price-model": expression})
	require.NoError(t, err)
	...
	c, info := newTaskSubmitContext(t, "param-price-model", "")
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/videos", strings.NewReader(`{"model":"param-price-model","seconds":8}`))
	c.Request.Header.Set("Content-Type", "application/json")
	c.Set("task_request", map[string]any{"seconds": float64(8)})
	c.Set("group", "default")
	info.UserGroup = "default"
	info.UsingGroup = "default"
	info.OriginModelName = "param-price-model"
	pinMappingOrderPlugin(t, c, requestParamTaskPlugin)
	...
	assert.Equal(t, billingexpr.BillingUnitRequest, snapshot.EstimatedBillingUnit)
	require.NotNil(t, snapshot.EstimatedFixedPrice)
	assert.InDelta(t, 0.05, *snapshot.EstimatedFixedPrice, 1e-9)
	assert.Equal(t, map[string]any{"seconds": float64(8)}, snapshot.ParamFacts)
	assert.Equal(t, int(0.05*8*common.QuotaPerUnit), info.PriceData.Quota)

	// Settlement re-runs the frozen expression without the request body.
	settled, settleErr := billingexpr.ComputeTieredQuotaWithRequest(snapshot, billingexpr.TokenParams{}, billingexpr.RequestInput{
		Usage:  snapshot.UsageFacts,
		Params: snapshot.ParamFacts,
	})
	require.NoError(t, settleErr)
	assert.Equal(t, info.PriceData.Quota, settled.ActualQuotaAfterGroup)
	assert.Equal(t, "per_second", settled.MatchedTier)
}

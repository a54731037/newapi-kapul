...
func settleTaskBillingOnComplete(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, taskResult *relaycommon.TaskInfo) bool {
	if bc := task.PrivateData.BillingContext; bc != nil && bc.TieredSnapshot != nil {
		...
		// ParamFacts are frozen at submission: the request body no longer exists
		// here, and request-derived prices must not change at settlement.
		result, err := billingexpr.ComputeTieredQuotaWithRequest(bc.TieredSnapshot, billingexpr.TokenParams{}, billingexpr.RequestInput{
			Usage:  usageFacts,
			Params: bc.TieredSnapshot.ParamFacts,
		})
		if err != nil {

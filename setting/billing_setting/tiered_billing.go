...
// Request-priced (fixed/per_call) leaves are allowed: their request-derived
// factors are frozen on the billing snapshot at submission and replayed at
// settlement, so a per-second or per-count price stays stable.

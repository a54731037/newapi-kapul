...
func TestSmokeTestTaskExprValidatesDeclaredUsageVectors(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "request-priced task leaves are accepted and settle from frozen facts",
			schema:     videoSchema,
			expression: `u("mode") == "pro" ? tier("per_second", per_call(0.4)) * u("seconds") : tier("std", per_call(0.2)) * u("seconds")`,
		},
		{

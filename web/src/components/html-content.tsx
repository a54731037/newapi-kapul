...
function IsolatedHtmlContent(props: {
  ...
  useEffect(() => {
    ...
    // Intercept hash-link clicks inside Shadow DOM so they scroll
    // the target element into view within the shadow tree.
    const handleAnchorClick = (e: MouseEvent) => {
      ...
      // Use attribute selector for maximum compatibility across browsers
      const targetId = href.slice(1)
      const target = wrapper.querySelector(`[id="${targetId}"]`)
      if (!target) return

...
export function SignUpForm({
  ...
  // 表单的 aff_code 初始值来自 localStorage，而邀请链接里的 ?aff= 只有到这里才写入
  // localStorage。冷访问（首次打开邀请链接）时初始值必然为空，因此必须显式回填字段，
  // 否则提交时不带 aff_code，邀请关系与双方奖励都会丢失。
  useEffect(() => {
    const aff = new URLSearchParams(window.location.search).get('aff')?.trim()
    if (!aff) return
    saveAffiliateCode(aff)
    if (form.getValues('aff_code') !== aff) {
  ...
  async function onSubmit(data: z.infer<typeof registerFormSchema>) {
    ...
    try {
      const res = await register({
        ...
        aff_code: data.aff_code || undefined,
        turnstile: turnstileToken,
      })

      if (res?.success) {
        // 邀请码只对本次注册生效，避免同一浏览器后续注册继续归属到旧邀请人
        clearAffiliateCode()
        toast.success(t('Account created! Please sign in'))
        redirectToLogin()
      } else {
        if (res.message === 'user.invite_code_invalid') {
          form.setError('aff_code', {
            message: t('Invalid invitation code'),
          })
        } else {
          handleServerError(

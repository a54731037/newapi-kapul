...
describe('local billing expression evaluation', () => {
  ...
  test('keeps a per_call request price through an unedited visual round trip', () => {
    const source = 'tier("1k", per_call(0.0300))'
    const document = parseVisualBillingDocument(source)
    assert(document)
    expect(serializeVisualBillingDocument(document)).toEqual({
      ok: true,
  ...
  test('represents request-probe resolution tiers visually and round-trips them unchanged', () => {
    const source = `(
  param("resolution") == "1k" ||
  (
    param("resolution") != "2k" &&
  ...
  : (
      ...
      (
        param("resolution") != "1k" &&
        param("resolution") != "4k" &&
        param("size") != "4096x4096" &&
        param("size") != "3840x2160" &&
        param("size") != "2160x3840"
      )
    )
  ...
  test('keeps request probe keys, text values and nil comparisons distinct', () => {
    const source =
      'param("resolution") == "2k" ? tier("2k", per_call(0.03)) : param("resolution") == nil ? tier("none", per_call(0.01)) : tier("4k", per_call(0.05))'
    const document = parseVisualBillingDocument(source)
    assert(document)
    expect(serializeVisualBillingDocument(document)).toEqual({
      ok: true,
    ...
    // Rewriting one price must keep the request conditions intact.
    const root = document.root
    assert(root.kind === 'branch' && root.no.kind === 'branch')
    const yes = root.yes
    assert(yes.kind === 'tier')
    expect(
      serializeVisualBillingDocument({
        ...document,
        root: { ...root, yes: { ...yes, fixedPrice: '0.04' } },
      })
    ).toEqual({
      ok: true,
      source: source.replace(
        'tier("2k", per_call(0.03))',
        'tier("2k", per_call(0.04))'
      ),
    })
  })
  test('refuses request conditions the visual grammar cannot express', () => {
    // Ordering a text value, or a dynamic key, must fall back to raw text.
    expect(
      parseVisualBillingDocument(
        'param("mode") > "a" ? tier("x", per_call(0.01)) : tier("y", per_call(0.02))'
      )
    ).toBeNull()
    expect(
      parseVisualBillingDocument(
        'param(dynamic_path) == "a" ? tier("x", per_call(0.01)) : tier("y", per_call(0.02))'
      )
    ).toBeNull()
  })
  test('reports a dynamic factor on the simulated request rules', () => {
    const result = evaluateBillingExpression(
      'tier("per_second", per_call(0.05)) * param("seconds")',
      { request: { body: { seconds: 8 } } }
    )
    ...
    expect(result.requestRules).toEqual([
      {
        cond: 'param("seconds")',
        multiplier: 8,
        matched: true,
  ...
  test('represents a request multiplier while rejecting invalid request prices', () => {
    const source = 'tier("per_second", per_call(0.05)) * param("seconds")'
    const document = parseVisualBillingDocument(source)
    assert(document)
    expect(document.multiplier).toMatchObject({
      kind: 'multiplier',
      probe: 'param',
      key: 'seconds',
    })
    ...
    for (const invalid of [
      ...
      'tier("bad", per_call(0.01) * param("n"))',
      'tier("bad", per_call(0.01)) + p * 2',
      'tier("bad", p * per_call(0.01))',
    ]) {
      expect(compileBillingExpression(invalid).status).not.toBe('ready')
    }
  })
...
describe('flat tier conditions with request fields', () => {
  const requestTierConfig = (
    condition: VisualTier['conditions']
  ): VisualConfig => ({
    tiers: [
      normalizeVisualTier({
        label: '1k',
  ...
  test('round-trips a text request condition through the flat config', () => {
    const config = requestTierConfig([
      { var: 'param', op: '==', value: '1024x1024', key: 'size', valueKind: 'string' },
    ])
    const expression = generateExprFromVisualConfig(config)
    expect(expression).toBe(
      'param("size") == "1024x1024" ? tier("1k", fixed(0.03)) : tier("2k", fixed(0.05))'
    )
  ...
  test('round-trips numeric, header and usage-fact conditions', () => {
    const config = requestTierConfig([
      { var: 'header', op: '==', value: 'pro', key: 'x-tier', valueKind: 'string' },
    ])
    config.tiers[0].conditions.push({
      var: 'u',
      op: '>=',
      value: 5,
      key: 'seconds',
    ...
    const expression = generateExprFromVisualConfig(config)
    expect(expression).toBe(
      'header("x-tier") == "pro" && u("seconds") >= 5 ? tier("1k", fixed(0.03)) : tier("2k", fixed(0.05))'
    )
    expect(tryParseVisualConfig(expression)?.tiers[0].conditions).toEqual([
      { var: 'header', op: '==', value: 'pro', key: 'x-tier', valueKind: 'string' },
      { var: 'u', op: '>=', value: 5, key: 'seconds', valueKind: 'number' },
    ])
  })

  test('round-trips a nil comparison and an explicitly empty text value', () => {
    const nilConfig = requestTierConfig([
      { var: 'param', op: '!=', value: '', key: 'resolution', valueKind: 'nil' },
    ])
    expect(generateExprFromVisualConfig(nilConfig)).toBe(
      'param("resolution") != nil ? tier("1k", fixed(0.03)) : tier("2k", fixed(0.05))'
    )
    expect(tryParseVisualConfig(generateExprFromVisualConfig(nilConfig))?.tiers[0].conditions).toEqual([
      { var: 'param', op: '!=', value: '', key: 'resolution', valueKind: 'nil' },
    ])

    const emptyConfig = requestTierConfig([
      { var: 'param', op: '==', value: '', key: 'size', valueKind: 'string' },
    ])
    expect(generateExprFromVisualConfig(emptyConfig)).toContain('param("size") == ""')
    expect(tryParseVisualConfig(generateExprFromVisualConfig(emptyConfig))?.tiers[0].conditions).toEqual([
      { var: 'param', op: '==', value: '', key: 'size', valueKind: 'string' },
    ])
  })

  test('drops incomplete request conditions instead of emitting invalid syntax', () => {
    const config = requestTierConfig([
      { var: 'param', op: '==', value: '1024x1024', valueKind: 'string' },
    ])
    expect(generateExprFromVisualConfig(config)).toBe(
      'tier("1k", fixed(0.03)) : tier("2k", fixed(0.05))'
    )
  })

  test('refuses an ordering comparison on a text value', () => {
    expect(
      tryParseVisualConfig(
        'param("size") < "b" ? tier("1k", fixed(0.03)) : tier("2k", fixed(0.05))'
      )
    ).toBeNull()
  })

  test('round-trips the free request fallback generated for a conditional single tier', () => {
    const expression =
      'param("size") == "1024x1024" ? tier("base", fixed(0.03)) : tier("no_match", fixed(0))'
    const parsed = tryParseVisualConfig(expression)
    assert(parsed)
  ...
  test('round-trips a whole-tree request multiplier on the flat config', () => {
    const config: VisualConfig = {
      ...
      multiplier: { probe: 'param', key: 'seconds', mode: 'value', units: [] },
    }
    const expression = generateExprFromVisualConfig(config)
    // The parenthesis is required so the factor scales every branch.
    expect(expression).toBe('(tier("per_second", fixed(0.05))) * param("seconds")')
  ...
  test('round-trips an enumerated unit list on the flat config', () => {
    ...
    const expression = generateExprFromVisualConfig(config)
    expect(expression).toBe(
      '(tier("per_second", fixed(0.05))) * (param("seconds") == "2" ? 2 : 1) * (param("seconds") == "3" ? 3 : 1) * (param("seconds") == "4" ? 4 : 1)'
    )
    const parsed = tryParseVisualConfig(expression)
    assert(parsed)
    expect(parsed.multiplier).toEqual({
  ...
  test('drops unusable unit values instead of making a duration free', () => {
    const config: VisualConfig = {
      ...normalizeVisualConfig({
        tiers: [
          normalizeVisualTier({
    ...
    // A zero factor would bill that duration nothing; the value factor is the
    // bounded fallback when every listed unit is unusable.
    expect(generateExprFromVisualConfig(config)).toBe(
      '(tier("per_second", fixed(0.05))) * (param("seconds") == "8" ? 8 : 1)'
    )
    expect(
      generateExprFromVisualConfig({
        ...config,
        multiplier: { probe: 'param', key: 'seconds', mode: 'units', units: [0] },
      })
    ).toBe('(tier("per_second", fixed(0.05))) * param("seconds")')
  })

  test('scales every resolution tier with one image-count multiplier', () => {
    const tiers = normalizeVisualConfig({
      tiers: [
        normalizeVisualTier({
          label: '1k',
          conditions: [
    ...
    const expression = generateExprFromVisualConfig({
      ...tiers,
      multiplier: { probe: 'param', key: 'n', mode: 'value', units: [] },
    })
    expect(expression).toBe(
      '(param("size") == "1024x1024" ? tier("1k", fixed(0.03)) : tier("4k", fixed(0.05))) * param("n")'
    )
    const parsed = tryParseVisualConfig(expression)
    assert(parsed)
    expect(parsed.multiplier).toEqual({
  ...
  test('leaves an unwrapped factor to the document editor', () => {
    // `a ? b : c * param("n")` only scales the else branch; it must not be
    // mistaken for a whole-tree multiplier.
    expect(
      tryParseVisualConfig(
        'param("size") == "1024x1024" ? tier("1k", fixed(0.03)) : tier("4k", fixed(0.05)) * param("n")'
      )
  ...
  test('rejects enumerated factors that are not per-unit or mix probes', () => {
    // `== "2" ? 3 : 1` is not a per-unit factor.
    expect(
      tryParseVisualConfig(
        '(tier("x", fixed(0.05))) * (param("seconds") == "2" ? 3 : 1)'
      )
    ).toBeNull()
    // Mixed probes cannot be one unit list.
    expect(
      tryParseVisualConfig(
        '(tier("x", fixed(0.05))) * (param("seconds") == "2" ? 2 : 1) * (u("seconds") == "3" ? 3 : 1)'
      )
    ).toBeNull()
  })

  test('represents the enumerated per-second preset in the document model', () => {
    const source = [
      'tier("per_second", per_call(0.05))',
      '  * (param("seconds") == "2" ? 2 : 1)',
      '  * (param("seconds") == "8" ? 8 : 1)',
      '  * (param("seconds") == "15" ? 15 : 1)',
    ].join('\n')
    const document = parseVisualBillingDocument(source)
    assert(document)
    expect(document.multiplier).toMatchObject({
    ...
    expect(serialized.source.replaceAll(/\s+/g, ' ')).toBe(
      'tier("per_second", per_call(0.05)) * (param("seconds") == "2" ? 2 : 1) * (param("seconds") == "8" ? 8 : 1) * (param("seconds") == "15" ? 15 : 1)'
    )
  })
})

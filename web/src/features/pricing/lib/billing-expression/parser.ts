...
import {
  ...
  TOKEN_VARIABLES,
  TIME_FUNCTIONS,
  REQUEST_PRICE_FUNCTIONS,
  REQUEST_PROBE_FUNCTIONS,
  ...
  visitExpression(ast, (node) => {
    ...
    if (node.kind === 'literal') {
      ...
      if (node.name === 'header') result = 'string'
        ...
        ['fixed', 'per_call', 'min', 'max', 'abs', 'ceil', 'floor'].includes(
          node.name
        )
      ) {
...
function usesRequestPrice(functions: ReadonlySet<string>): boolean {
  return REQUEST_PRICE_FUNCTIONS.some((name) => functions.has(name))
}

function containsPricingMarker(node: ExpressionNode): boolean {
  const calls = expressionDependencies(node).functions
  return (
    calls.has('tier') || REQUEST_PRICE_FUNCTIONS.some((name) => calls.has(name))
  )
}

// A dynamic factor reads one request value, e.g. per_call(0.05) * param("seconds").
function isDynamicRequestFactor(node: ExpressionNode): boolean {
  return (
    node.kind === 'call' &&
    (REQUEST_PROBE_FUNCTIONS as readonly string[]).includes(node.name) &&
    node.args.every(
      (arg) => arg.kind === 'literal' && typeof arg.value === 'string'
    )
  )
}

// The dynamic factor of a multiplication, when the other side carries the price.
function dynamicFactorOf(
  node: Extract<ExpressionNode, { kind: 'binary' }>
): ExpressionNode | undefined {
  if (node.operator !== '*') return undefined
  if (isDynamicRequestFactor(node.right) && containsPricingMarker(node.left)) {
    return node.right
  }
  if (isDynamicRequestFactor(node.left) && containsPricingMarker(node.right)) {
    return node.left
  }
  return undefined
}
...
function validateFixedPricingTree(
  ...
  if (node.kind === 'binary' && node.operator === '*') {
    for (const [factor, pricing] of [
      ...
      // Dynamic factors are bounded at run time instead of carrying a literal.
      if (isDynamicRequestFactor(factor) && containsPricingMarker(pricing)) {
        validateFixedPricingTree(pricing, rules)
        return
      }
    }
  ...
  if (
    ...
    if (price.kind === 'call' && isRequestPriceFunction(price.name)) {
  ...
  throw new BillingExpressionError({
    code: 'type',
    detail:
      'Use tier(name, fixed(amount)) or tier(name, per_call(amount)) with a finite, non-negative numeric literal; combine pricing branches only with conditions and request multipliers applied outside tier().',
    position: node.start,
  })
}
...
export function compileBillingExpression(source: string): CompilationResult {
  ...
  try {
    ...
    // One postorder pass, mirroring the engine's instrumentation order: literal
    // factors are traced for every expression, dynamic ones only alongside a
    // request price.
    const tracesDynamicFactors = usesRequestPrice(dependencies.functions)
    visitExpression(ast, (node) => {
      ...
      if (!tracesDynamicFactors) return
      if (node.kind !== 'binary') return
      const factor = dynamicFactorOf(node)
      if (!factor) return
      requestRules.push({
        node: factor,
        // Neutral until the factor is read; an unselected branch keeps this.
        multiplier: 1,
        cond: source.slice(factor.start, factor.end),
        dynamic: true,
      })
    })
    if (tracesDynamicFactors) {

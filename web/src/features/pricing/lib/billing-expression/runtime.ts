...
type BoundedMultiplier = {
  value: number
  matched: boolean
  clamped: boolean
}

function boundedFromNumber(numeric: number): BoundedMultiplier {
  if (!Number.isFinite(numeric)) {
    throw new BillingExpressionError({
      code: 'type',
      detail: 'request multiplier is not finite',
    })
  }
  if (numeric < 0) {
    throw new BillingExpressionError({
      code: 'type',
      detail: 'request multiplier must not be negative',
    })
  }
  if (numeric === 0) return { value: 1, matched: false, clamped: false }
  if (numeric > MAX_REQUEST_MULTIPLIER) {
    return { value: MAX_REQUEST_MULTIPLIER, matched: true, clamped: true }
  }
  return { value: numeric, matched: true, clamped: false }
}

/** Mirrors boundedRequestMultiplier in the Go engine. */
export function toRequestMultiplier(value: unknown): BoundedMultiplier {
  if (value === null || value === undefined) {
    return { value: 1, matched: false, clamped: false }
  }
  if (typeof value === 'string') {
    const trimmed = value.trim()
    if (trimmed === '') return { value: 1, matched: true, clamped: false }
    const parsed = Number(trimmed)
    if (!Number.isFinite(parsed)) {
      throw new BillingExpressionError({
        code: 'type',
        detail: 'request multiplier is not a number',
      })
    }
    return boundedFromNumber(parsed)
  }
  if (typeof value !== 'number') {
    throw new BillingExpressionError({
      code: 'type',
      detail: 'request multiplier',
    })
  }
  return boundedFromNumber(value)
}
...
class BillingRuntime {
  ...
  // Finds the traced dynamic factor of a multiplication, if any.
  private multiplicandRule(
    node: Extract<ExpressionNode, { kind: 'binary' }>
  ): { index: number; factor: ExpressionNode } | undefined {
    for (const factor of [node.right, node.left]) {
      const index = this.compiled.requestRules.findIndex(
        (rule) => rule.dynamic === true && rule.node === factor
      )
      if (index >= 0) return { index, factor }
    }
    return undefined
  }

  // Mirrors the engine: absent, empty and zero factors are neutral, unusable
  // values fail closed, and oversized values saturate at MAX_REQUEST_MULTIPLIER.
  private boundedMultiplier(ruleIndex: number, factor: ExpressionNode): number {
    const bounded = toRequestMultiplier(this.evaluate(factor).value)
    const rule = this.requestRules[ruleIndex]
    rule.matched = bounded.matched
    rule.multiplier = bounded.value
    rule.clamped = bounded.clamped
  ...
  private binary(
    ...
    if (node.operator === '*') {
      const dynamic = this.multiplicandRule(node)
      if (dynamic) {
        // Evaluate in source order, like the engine's instrumented program.
        const pricingNode = dynamic.factor === node.left ? node.right : node.left
        if (dynamic.factor === node.right) {
          const price = this.numeric(pricingNode).value
          return {
            value: price * this.boundedMultiplier(dynamic.index, dynamic.factor),
          }
        }
        const multiplier = this.boundedMultiplier(dynamic.index, dynamic.factor)
        return { value: this.numeric(pricingNode).value * multiplier }
      }
    }

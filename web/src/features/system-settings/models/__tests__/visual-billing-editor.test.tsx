...
describe('visual time billing editor', () => {
  ...
  test('edits request-probe resolution tiers through the visual condition tree', async () => {
    ...
    render(
      <TieredPricingEditor
        billingExpr='param("resolution") == "1k" ? tier("1k", per_call(0.03)) : tier("2k", per_call(0.05))'
        requestRuleExpr=''
        onBillingExprChange={onBillingExprChange}
        onRequestRuleExprChange={vi.fn()}
      />
    ...
    // The request condition is editable instead of falling back to raw text.
    expect(
      screen.getByRole('textbox', { name: 'Request field path' })
    ...
    expect(onBillingExprChange).toHaveBeenLastCalledWith(
      'param("quality") == "1k" ? tier("1k", per_call(0.03)) : tier("2k", per_call(0.05))'
    )

    await user.click(
      screen.getByRole('combobox', { name: 'Condition value type' })
    )
    await user.click(screen.getByRole('option', { name: 'Not provided' }))
    expect(onBillingExprChange.mock.lastCall?.[0]).toContain(
      'param("quality") == nil'
    )
    expect(
      screen.queryByRole('textbox', { name: 'Condition value' })
    ).not.toBeInTheDocument()
  })
  test('seeds request-field resolution tiers from a preset and opens them visually', async () => {
    const onBillingExprChange = vi.fn()
    const user = userEvent.setup()
    render(
      <TieredPricingEditor
  ...
  test('adds a request-field condition in the tier editor and keeps it in the expression', async () => {
    ...
    render(
      <TieredPricingEditor
        billingExpr='tier("base", fixed(0.03))'
        requestRuleExpr=''
        onBillingExprChange={onBillingExprChange}
        onRequestRuleExprChange={vi.fn()}
      />
    ...
    await user.click(screen.getByRole('option', { name: 'Request field' }))

    // The default text value keeps the condition complete and editable.
    fireEvent.change(
      screen.getByRole('textbox', { name: 'Request field path' }),
      { target: { value: 'size' } }
    )
  ...
  test('seeds the video per-second price from a preset', async () => {
    const onBillingExprChange = vi.fn()
    const user = userEvent.setup()
    render(
      <TieredPricingEditor
    ...
    await user.click(screen.getByRole('button', { name: 'More templates...' }))
    await user.click(
      screen.getByRole('button', { name: 'Per second (per_call)' })
    )

    const emitted = onBillingExprChange.mock.lastCall?.[0] as string
    expect(emitted).toContain('tier("per_second", per_call(0.05))')
    expect(emitted).toContain('(param("seconds") == "15" ? 15 : 1)')
    // The enumerated factor is a visual structure now: one row per second count.
    expect(
      screen.getByRole('combobox', { name: 'Request multiplier mode' })
    ).toHaveTextContent('By unit list')
  ...
  test('adds a whole-tree request multiplier from the tier editor', async () => {
    ...
    render(
      <TieredPricingEditor
        billingExpr='tier("per_second", fixed(0.05))'
        requestRuleExpr=''
        onBillingExprChange={onBillingExprChange}
        onRequestRuleExprChange={vi.fn()}
      />
    ...
    await user.click(screen.getByRole('button', { name: 'Request multiplier' }))
    fireEvent.change(
      screen.getByRole('textbox', { name: 'Request field path' }),
  ...
  test('shows and removes a multiplier on the visual editor', async () => {
    ...
    render(
      <TieredPricingEditor
        billingExpr='(param("size") == "1024x1024" ? tier("1k", per_call(0.03)) : tier("2k", per_call(0.05))) * param("n")'
        requestRuleExpr=''
        onBillingExprChange={onBillingExprChange}
        onRequestRuleExprChange={vi.fn()}
      />
    ...
    // The multiplier row is rendered above the tree, so its key input comes first.
    expect(
      screen.getAllByRole('textbox', { name: 'Request field path' })[0]
    ).toHaveValue('n')
    expect(onBillingExprChange).not.toHaveBeenCalled()
  ...
  test('edits the enumerated per-second units visually', async () => {
    ...
    // The enumerated factor opens as a whole-tree unit list instead of raw text.
    expect(
      screen.getByRole('combobox', { name: 'Request multiplier mode' })
    ).toHaveTextContent('By unit list')
    expect(
      screen.getAllByRole('spinbutton', { name: 'Unit value' })
    ).toHaveLength(2)
    expect(onBillingExprChange).not.toHaveBeenCalled()

...
export function getSyncExpressionPricing(expression: string, t: (key: string) => string) {
  ...
  const tokenLabels: Record<'p' | 'c' | 'len', string> = {
    p: t('Input tokens'),
    c: t('Output tokens'),
    len: t('Length'),
  }
  // Request conditions keep their expression spelling: they have no localized
  // label, and rewriting them here would misreport what gets stored.
  const describeCondition = (condition: TierConditionInput): string => {
    if (!isTierRequestConditionVar(condition.var)) {
      return `${tokenLabels[condition.var]} ${condition.op} ${Number(condition.value).toLocaleString()}`
    }
    const valueKind = condition.valueKind ?? 'number'
    let value: string
    if (valueKind === 'nil') value = 'nil'
    else if (valueKind === 'string') value = JSON.stringify(String(condition.value ?? ''))
    else value = String(condition.value)
    return `${condition.var}(${JSON.stringify(condition.key ?? '')}) ${condition.op} ${value}`
  }
  return {
    requestRuleExpr,

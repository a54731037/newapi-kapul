...
function conditionRange(
  ...
  if (
    ...
    (start.key ?? '') !== (end.key ?? '') ||
    (start.valueKind ?? 'number') !== 'number' ||
    (end.valueKind ?? 'number') !== 'number' ||
    ...
    <>
      <div className='flex min-w-0 flex-1 flex-wrap items-center gap-2'>
        <BillingTimeProbeFields
          ...
          keyValue={comparison.key ?? ''}
          invalidKey={errors.some(
            (issue) => issue.message === 'Enter a request field.'
          )}
          ...
          onKeyChange={(key) => {
            if (node.kind === 'comparison') {
              ...
              props.onChange({
                ...node,
                children: range.map((bound) => ({ ...bound, key })),
              })
            }
          }}
          onChange={(probe, timezone) => {
            // Non-request probes keep the numeric grammar, so drop request-only
            // fields instead of leaving a string kind behind.
            const next = isRequestProbe(probe)
              ? { probe, timezone }
              : { probe, timezone, key: '', valueKind: 'number' as const }
            if (node.kind === 'comparison') {
              props.onChange({ ...node, ...next })
            } else if (range && (node.kind === 'all' || node.kind === 'any')) {
              props.onChange({
                ...node,
                children: range.map((bound) => ({ ...bound, ...next })),
              })
            }
          }}
        />
        {node.kind === 'comparison' && (
          <>
            ...
            />
            {isRequestProbe(node.probe) && !range && (
              <BillingValueKindSelect
                value={node.valueKind ?? 'number'}
                onChange={(valueKind) => {
                  const equality = node.operator === '!=' ? '!=' : '=='
                  props.onChange({
                    ...node,
                    valueKind,
                    value: valueKind === 'nil' ? '' : node.value,
                    operator:
                      valueKind === 'number' ? node.operator : equality,
                  })
                }}
              />
            )}
            {(node.valueKind ?? 'number') !== 'nil' && (
              <BillingConditionValueInput
                probe={node.probe}
                valueKind={node.valueKind ?? 'number'}
                value={node.value}
                invalid={errors.some(
